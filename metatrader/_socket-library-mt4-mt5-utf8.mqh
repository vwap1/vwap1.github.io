...
ClientSocket::ClientSocket(string HostnameOrIPAddress, ushort port)
{
   ...
   uchar arrName[];
   StringToCharArray(HostnameOrIPAddress, arrName, 0, -1, CP_UTF8);
...
bool ClientSocket::Send(string strMsg)
{
   ...
   bool bRetval = true;
   uchar arr[];
   StringToCharArray(strMsg, arr, 0, -1, CP_UTF8);
...
string ClientSocket::Receive(string MessageSeparator = "")
{
   ...
   if (TerminalInfoInteger(TERMINAL_X64)) {
      ...
      while (res > 0) {
         ...
         if (res > 0) {
            StringAdd(mPendingReceiveData, CharArrayToString(arrBuffer, 0, res, CP_UTF8));
